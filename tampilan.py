...
def tampilkan_status(peliharaan: Peliharaan, pemain: Pemain, nama_periode: str):
    ...
    print("╠" + "═"*48 + "╣")
    print(f"║  🧮 Skor Kebugaran: {skor:.1f} / 100  ({kategori:<1})  {"":<5} ║")
...
def tampilkan_navigasi_histori(peliharaan: Peliharaan):
    ...
    while True:
        ...
        if snapshot is None:
            ...
            print(f"║  Hari ke-{hari_ke}".ljust(49) + "║")
            print("║" + "  Statistik (saat dicatat):".ljust(48) + "║")
            usia_tampil = snapshot.get("usia", 0.0)
            print(f"║    Usia      : {usia_tampil:.1f} hari".ljust(49) + "║")
            print(f"║    Kelaparan : {snapshot.get('kelaparan', 0):.1f}".ljust(49) + "║")
            print(f"║    Kesenangan: {snapshot.get('kesenangan', 0):.1f}".ljust(49) + "║")
            print(f"║    Kesehatan : {snapshot.get('kesehatan', 0):.1f}".ljust(49) + "║")
            print(f"║    Berat     : {snapshot.get('berat', 0):.1f}".ljust(49) + "║")
            print(f"║    Energi    : {snapshot.get('energi', 0):.1f}".ljust(49) + "║")
            print(f"║    Evolusi   : {snapshot.get('tahap_evolusi', '?')}".ljust(49) + "║")
        print("╠" + "═"*48 + "╣")
